  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message: 'User not found. Please login or register as a user.'
      });
    }
...
const approveAttendance = async (req, res) => {
  ...
  try {
    if (!year || !month) {
      return res.status(400).json({
        message: 'Invalid parameters. Please provide year and month.'
      });
    }
    ...
    if (user.role !== 'Manager' && user.role !== 'Admin') {
      return res.status(403).json({
        message: 'Access denied. Only Manager or Admin can approve attendance.'
      });
    }
    const dateFilter = {
...
const approveSingleAttendance = async (req, res) => {
  ...
  try {
    if (!year || !month || !selectedIds) {
      return res.status(400).json({
        message: 'Invalid parameters. Please provide year, month, and selectedIds.'
      });
    }
    const user = await prisma.user.findUnique({
      where: {
...
const getDailyClockStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: 'Error checking daily clock status'
    });
  }
};
const getTodaysAttendance = async (req, res) => {
  try {
    ...
    const todaysAttendance = employees.map(employee => {
      const attendance = attendanceRecords.find(record => record.employeeId === employee.employee_id);
      return {
